...
public interface ProjectRepository extends JpaRepository<Project, Long> {
     ...
     * - 리워드의 옵션 그룹과 옵션 값은 @BatchSize를 통해 배치 로딩됨 (MultipleBagFetchException 방지).
     */
    @Query("""
            SELECT DISTINCT p
            FROM Project p
     ...
     * 결과: Object[] {projectId, projectName, makerId, makerName, category,
     * goalAmount, endDate, resultStatus, fundingAmount, supporterCount,
     * bookmarkCount}
     */
    @Query("""
            SELECT p.id,
